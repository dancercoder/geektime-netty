 ...
 *
 * ChannelPipeline是一串ChannelHandler，这些handler用来处理、拦截Channel上入站event和出站operation。
 * ChannelPipeline使用了职责链模式，方便用户控制event的处理方式和pipeline中ChannelHandler之间交互的方式。
 *
 * <h3>Creation of a pipeline</h3>
 *
 * Each channel has its own pipeline and it is created automatically when a new channel is created.
 * 每个channel都有自己的pipeline，是在channel被创建的时候就自动创建的。
 *
 * <h3>How an event flows in a pipeline</h3>
 * event在pipeline中是怎么移动的呢？
 *
 ...
 *
 * 以下图示展示了IO事件在pipeline中怎样被channelHandler处理。
 * IO事件会被ChannelInboundHandler处理，或者被ChannelOutboundHandler处理。处理完毕后，event被向前传递给最近的下一个
 * handler（通过调用ChannelHandlerContext中定义的传播方法实现）
 * 比如，ChannelHandlerContext的fireChannelRead(Object)方法、ChannelHandlerContext的write(Object)方法
 *
 * <pre>
 *                                                 I/O Request
 *                                            via {@link Channel} or
 ...
 *  |                                                                   |
 *  |  Netty Internal I/O Threads (Transport Implementation)传输层实现    |
 *  +-------------------------------------------------------------------+
 *  从图上可以看到事件之间的传递处理，也就是说handler的顺序很重要；
 *  对应到多次编解码，编码、解码的handler要以相反的顺序放进去
 * </pre>
 ...
 * 入站event到达pipeline结束位置，会比丢弃，或者需要的话日志记录。
 * <p>
 ...
 * 出站event到达pipeline技术位置，通常会执行IO操作，事实上的output操作SocketChannel的write(ByteBuffer)
 * <p>
 ...
 * 实际addLast添加的Handler都是封装成为ChannelHandlerContext的；
 * DefaultChannelHandlerContext中封装了pipeline、executor、handler
 * 在以上给出的例子中，入站的event一次经过1-2-3-4-5的处理；出站的event一次经过5-4-3-2-1处理。
 * 在这个准则的基础上，pipeline会跳过被@Skip注解的handler，这样减少调用栈的深度
 * <p>
 ...
 * 一般而言，典型的服务端程序需要在pipeline中有以下的handler：
 * 协议解码handler
 * 协议编码handler
 * 业务逻辑handler
 *
 ...
 * 如果业务逻辑是异步的操作，或者执行可以很快结束，那么没有必要使用group；否则使用group使业务逻辑与IO线程隔离，
 * </pre>
 *
